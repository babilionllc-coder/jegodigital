...
class Continue200LeadsAnalyzer {
    constructor() {
        this.doc = null;
        this.browser = null;
    ...
    async initialize() {
        console.log('🚀 Initializing Continue 200 Leads Analyzer...');
    ...
    async getLeadsFromMainSheet(startIndex, count) {
        ...
        // Get leads from the main sheet with 2000+ leads
        ...
        for (let i = startIndex; i < Math.min(startIndex + count, rows.length); i++) {
            ...
            if (businessName && phoneNumber && phoneNumber.trim() !== '') {
                leadsToAnalyze.push({
                    ...
                    businessName,
                    phoneNumber,
                    location: row.get('Location') || 'Cancún, México',
...
Alex. CEO JegoDigital`;
    ...
    async analyzeBatch(startIndex, count) {
        ...
        for (let i = 0; i < leads.length; i++) {
            const lead = leads[i];
            console.log(`\n🎯 ANALYZING LEAD ${startIndex + i + 1}/${this.targetLeads}: ${lead.businessName}`);
    ...
    async runTo200Leads() {
        ...
        let totalProcessed = this.startIndex; // Start from 28
        let currentIndex = 0; // Start from beginning of main sheet
        
        while (totalProcessed < this.targetLeads) {
            try {
                const batchSize = Math.min(10, this.targetLeads - totalProcessed);
                const processed = await this.analyzeBatch(currentIndex, batchSize * 2); // Get more to filter for phone numbers
                ...
                console.log(`\n📊 PROGRESS: ${totalProcessed}/${this.targetLeads} leads processed`);
                
                if (totalProcessed >= this.targetLeads) {
...
// Main execution
async function main() {
    const analyzer = new Continue200LeadsAnalyzer();
...
module.exports = Continue200LeadsAnalyzer;
