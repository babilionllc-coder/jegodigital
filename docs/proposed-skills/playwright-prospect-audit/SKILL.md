...
description: Render any prospect website in headless Chromium to extract observable cold-email personalization signals — page weight, mobile load time, WhatsApp button presence, schema.org JSON-LD, copyright year, meta description, total request count, console errors, mobile screenshot. Picks the meanest defensible hook per prospect and produces a one-line personalization string that drops into Instantly cold-email templates as `{{personalization}}`. Works on any domain — Mexican real estate, Miami luxury, NYC, LA. Use EVERY TIME Alex wants to personalize cold email at scale by pre-rendering each prospect's site, batch-audit a CSV of leads, push enriched leads to Instantly with audit fields, or improve the per-prospect hook quality. Triggers — playwright audit, site audit for cold email, batch audit prospects, render prospect site, personalization hook, audit_prospect.mjs, batch_audit.mjs, rerank_for_miami, audit_and_upload, mobile screenshot prospects, page weight personalization, WhatsApp button check, observable hook, cold email personalization at scale.
---
