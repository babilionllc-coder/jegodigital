...
class NonStopAnalyzer {
    constructor() {
        ...
        this.analyzedLeads = [];
        this.batchSize = 10; // Process 10 leads at a time
    }

    async initialize() {
        console.log('🚀 Initializing Non-Stop Analyzer...');
    ...
    async getLeadsToAnalyze(startIndex = 0, count = 50) {
        console.log(`📋 Getting leads ${startIndex + 1} to ${startIndex + count}...`);
        ...
        for (let i = startIndex; i < Math.min(startIndex + count, rows.length); i++) {
            ...
            leadsToAnalyze.push({
                ...
                industry: row.get('Business Type')
            });
        }
...
Alex. CEO JegoDigital`;
    ...
    async analyzeBatch(startIndex, count) {
        ...
        const leads = await this.getLeadsToAnalyze(startIndex, count);
        
        for (let i = 0; i < leads.length; i++) {
            const lead = leads[i];
            console.log(`\n🎯 ANALYZING LEAD ${startIndex + i + 1}/${startIndex + leads.length}: ${lead.businessName}`);
        ...
        console.log(`\n🎉 BATCH ${Math.floor(startIndex/10) + 1} COMPLETE!`);
        console.log(`📊 Processed ${this.analyzedLeads.length} total leads`);
    ...
    async uploadResults() {
        ...
        // Get current rows to append new ones
        const currentRows = await leadsSheet.getRows();
        const startRow = currentRows.length + 2; // Start after existing data
        ...
        console.log(`🎉 Successfully uploaded ${newRows.length} new results!`);
        console.log('📊 Check the "Complete Real Analysis" sheet in your Google Sheets');
    ...
    async runNonStop() {
        console.log('🚀 Starting NON-STOP analysis of all leads...');
        
        let totalProcessed = 0;
        let currentIndex = 28; // Start from where we left off
        
        while (totalProcessed < 200) {
            try {
                const batchSize = Math.min(10, 200 - totalProcessed);
                const processed = await this.analyzeBatch(currentIndex, batchSize);
                ...
                console.log(`\n📊 PROGRESS: ${totalProcessed}/200 leads processed`);
    ...
    async close() {
        ...
        console.log('🔧 Non-Stop Analyzer closed');
    }
}

// Main execution
async function main() {
    const analyzer = new NonStopAnalyzer();
    
    try {
        await analyzer.initialize();
        await analyzer.runNonStop();
        
    } catch (error) {
        console.error('❌ Error during non-stop analysis:', error);
    } finally {
...
module.exports = NonStopAnalyzer;
