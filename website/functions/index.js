...
// --- Post-call WhatsApp follow-up for FB broker AI cold calls ---
// ElevenLabs Offer D agent (agent_7301kq5jxe0gf3vbmp92c974stzc) webhooks here
// after each call. Polite-exit + interested-whatsapp outcomes get queued
// into /whatsapp_followup_queue with a Telegram alert + wa.me deeplink to Alex.
    ...
    try {
        ...
        if (BREVO_KEY) {
            try {
                ...
                notifications.brevo = { ok: true, listId: targetListId, at: nowIso() };
            } catch (brevoErr) {
                const e = brevoErr.response?.data || brevoErr.message;
                functions.logger.warn("⚠️ Brevo add failed (lead still in Firestore):", e);
                notifications.brevo = { ok: false, error: String(e).slice(0, 500), at: nowIso() };
            }
        } else {
            functions.logger.warn("⚠️ BREVO_API_KEY not set — skipping Brevo add");
            notifications.brevo = { ok: false, skipped: true, reason: "no_brevo_key", at: nowIso() };
        }
        ...
        try {
            ...
            notifications.telegram = { ok: true, at: nowIso() };
        } catch (e) {
            functions.logger.warn("telegramHelper failed:", e.message);
            notifications.telegram = { ok: false, error: e.message, at: nowIso() };
        }
        ...
        try {
            ...
            if (slackUrl) {
                ...
                notifications.slack = { ok: true, at: nowIso() };
            } else {
                functions.logger.warn("⚠️ SLACK_WEBHOOK_URL not set — skipping Slack alert");
                notifications.slack = { ok: false, skipped: true, reason: "no_webhook_url", at: nowIso() };
            }
        } catch (slackErr) {
            functions.logger.warn("⚠️ Slack post failed (non-fatal):", slackErr.response?.data || slackErr.message);
            notifications.slack = {
                ok: false,
                error: String(slackErr.response?.data || slackErr.message).slice(0, 500),
                at: nowIso()
            };
        }
        ...
        try {
            ...
            if (BREVO_KEY) {
                ...
                notifications.alex_email = { ok: true, to: alexEmail, at: nowIso() };
            } else {
                notifications.alex_email = { ok: false, skipped: true, reason: "no_brevo_key", at: nowIso() };
            }
        } catch (emailErr) {
            functions.logger.warn("⚠️ Alex email failed (non-fatal):", emailErr.response?.data || emailErr.message);
            notifications.alex_email = {
                ok: false,
                error: String(emailErr.response?.data || emailErr.message).slice(0, 500),
                at: nowIso()
            };
        }

        // 6. Flush delivery tracking to Firestore so auditNotificationWatchdog
        //    can surface silent failures within 15 min (Option B — HARD RULE #6).
        try {
            await docRef.update({
                notifications,
                notifications_updated_at: admin.firestore.FieldValue.serverTimestamp()
            });
        } catch (updateErr) {
            functions.logger.warn("notifications update failed (non-fatal):", updateErr.message);
        }
...
// replies), sends via Brevo SMTP transactional API. Cancels
// remaining touches if lead books Calendly (calendlyWebhook
// flips calendlyBooked=true on the brevo_nurture_index doc).
// Closes the "0 of 9 repliers in Brevo" gap found 2026-04-22 PM.
// ============================================================
const brevoNurture = require("./brevoNurture");
exports.processBrevoNurtureQueue = functions
    .runWith({ timeoutSeconds: 300, memory: "512MB" })
    .pubsub.schedule("every 30 minutes")
    .timeZone("America/Mexico_City")
    .onRun(async () => {
        try {
            const r = await brevoNurture.processNurtureQueue({ limit: 40 });
            functions.logger.info(
                `brevoNurtureQueue: processed=${r.processed} sent=${r.sent} ` +
                `skipped=${r.skipped} failed=${r.failed}`
            );
            return null;
        } catch (err) {
            functions.logger.error("brevoNurtureQueue run failed:", err.message);
            return null;
        }
    });

// On-demand HTTPS trigger (for manual kick / testing).
exports.processBrevoNurtureQueueOnDemand = functions.https.onRequest(async (req, res) => {
    try {
        const r = await brevoNurture.processNurtureQueue({ limit: Number(req.query.limit) || 40 });
        res.json({ ok: true, ...r });
    } catch (err) {
...
// ============================================================
// CREDIT / QUOTA WATCHDOGS (added 2026-04-24 PM)
// Polls ElevenLabs, Brevo, Twilio, Hunter, DataForSEO every 6h.
// Alerts #alerts via SLACK_BOT_TOKEN when threshold crossed.
//
// Why: 2026-04-24 ElevenLabs hit 91.8% and NO ONE knew. Cold-calls
// were about to silently fail mid-call. This prevents that pattern.
// See creditWatchdogs.js for thresholds + alert copy.
// ============================================================
...
// ============================================================
// Sofia WhatsApp Cloud API inbound (Meta WABA 1520533496454283,
// phone "+1 978-396-7234", phone_number_id 1044375245434120).
// Handles GET verify handshake + POST inbound messages → routes
// to Gemini Sofia → replies via Cloud API + alerts on hot leads.
// Endpoint: /whatsappCloudInbound
// Setup: Alex must update WABA webhook URL in Meta UI (currently
// pointing to abandoned jegodigital-2ed98 project).
// ============================================================
const whatsappCloudInboundMod = require("./whatsappCloudInbound");
exports.whatsappCloudInbound = whatsappCloudInboundMod.whatsappCloudInbound;
