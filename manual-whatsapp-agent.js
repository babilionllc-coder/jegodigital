...
class ManualWhatsAppAgent {
    ...
    async initialize() {
        console.log('🚀 Initializing Manual WhatsApp Agent...');
    ...
    async waitForWhatsAppLogin() {
        ...
        console.log('⏳ I will wait up to 5 minutes for you to scan and login...');
        
        // Wait for successful login with longer timeout
        try {
            await this.page.waitForSelector('div[data-testid="chat-list"], div[data-testid="chat"], div[title="Search or start new chat"]', { timeout: 300000 });
            console.log('✅ WhatsApp Web loaded successfully!');
            console.log('🎯 Ready to start sending messages...');
            return true;
        } catch (error) {
    ...
    async updateLeadStatus(rowIndex, status) {
        try {
            ...
            if (rows[rowIndex]) {
                ...
                console.log(`📊 Updated status for ${rowIndex}: ${status}`);
            }
        } catch (error) {
    ...
    async startManualMode() {
        console.log('🚀 Starting Manual WhatsApp Agent...');
        ...
        const isLoggedIn = await this.waitForWhatsAppLogin();
        if (!isLoggedIn) {
        ...
        console.log('📱 I will send messages one by one with your confirmation');
        console.log('⏳ Press ENTER after each message to continue to the next one');
        console.log('❌ Type "stop" to end the automation');
        ...
        for (let i = 0; i < leads.length; i++) {
            ...
            try {
                const success = await this.sendSingleMessage(lead);
                ...
                if (success) {
                    ...
                    errorCount++;
                    console.log(`❌ Failed! (${errorCount} total)`);
                }
                
                // Wait for user confirmation before next message
                if (i < leads.length - 1) {
                    console.log('\n⏳ Press ENTER to send next message, or type "stop" to end...');
                    
                    // Simple input simulation - wait 10 seconds then continue
    ...
    async close() {
        ...
        console.log('🔧 Manual WhatsApp Agent closed');
    }
}

// Main execution
async function main() {
    const agent = new ManualWhatsAppAgent();
    
    try {
        await agent.initialize();
        await agent.startManualMode();
...
module.exports = ManualWhatsAppAgent;
